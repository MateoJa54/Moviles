...
class FechaInput extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        Expanded(
          child: NumberField(controller: dia, hint: 'Día'),
        ),
        const SizedBox(width: 8),
        Expanded(
          child: NumberField(controller: mes, hint: 'Mes'),
        ),
        const SizedBox(width: 8),
        Expanded(
          child: NumberField(controller: anio, hint: 'Año'),
        ),
      ],
    );
  }
...
class EdadPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: const Padding(padding: EdgeInsets.all(16), child: EdadCard()),
    );
  }
}
